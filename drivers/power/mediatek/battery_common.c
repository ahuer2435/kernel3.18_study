...
extern int cm_get_Temperature(void);
extern int cm_get_Voltage(void);
extern int cm_get_StateOfCharge(void);
extern int cm_get_AverageCurrent(void);
...
extern void hw_otg_reset_aca_type(void);  // add @lgx 
kal_bool ycable_charging_on = KAL_TRUE ;
...
kal_bool upmu_is_chr_det(void)
{
	...
	if (tmp32 == 0)
	{
		...
		if (mt_usb_is_device()) {
			...
			printk("[upmu_is_chr_det] Charger exist and USB is not host\n");
			return KAL_TRUE;
		} else if (0x4 == ACA_type) {
			printk("[upmu_is_chr_det] ACA Cable plug in\n");
			BMT_status.charger_type = STANDARD_CHARGER;
			if (KAL_FALSE == ycable_charging_on)
				BMT_status.bat_in_charging_enable = KAL_FALSE;
			else
			...
			printk("[upmu_is_chr_det] Charger exist but USB is host\n");
			return KAL_FALSE;
		}
	#else
		return KAL_TRUE;
...
#if !defined(CONFIG_POWER_EXT)
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // Create File For Y-cable-charging : Ycable_charging_enable */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_Ycable_charging_enable(struct device *dev, struct device_attribute *attr,
                   char *buf)
{
   printk("[Battery] show_Ycable_charging_enable : %d\n",
               BMT_status.bat_in_charging_enable);
   return sprintf(buf, "%d\n", BMT_status.bat_in_charging_enable);
}

static ssize_t store_Ycable_charging_enable(struct device *dev, struct device_attribute *attr,
                    const char *buf, size_t size)
{
   unsigned int charging_enable = 0;
   unsigned int i_ACA_type = 0;
   printk("[Battery] store_Ycable_charging_enable\n");
   i_ACA_type = hw_otg_get_aca_type();
   if (i_ACA_type != 4)
        {   
            printk("[Battery] store_Ycable_charging_enable: no ycable cable in, do no set charging enable\n");
            return size;
        }
   if (buf != NULL && size != 0) {
       charging_enable = simple_strtoul(buf, NULL, 32);
       if ((0 == charging_enable) || (1 == charging_enable)) {
           BMT_status.bat_in_charging_enable = charging_enable;
		   ycable_charging_on = charging_enable ;
		   printk("--@lgxc-----y3----==%d\n",charging_enable);
               battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
       } else
           printk("[Battery] store_Ycable_charging_enable: invalid input\n");
   }
   return size;
}

static DEVICE_ATTR(Ycable_charging_enable, 0664, show_Ycable_charging_enable, store_Ycable_charging_enable);

static ssize_t show_thub_status(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	battery_log(BAT_LOG_CRTI, "show_thub_status : %d\n", hw_otg_get_aca_type());
	return sprintf(buf, "%d\n", hw_otg_get_aca_type());
}

static ssize_t store_thub_status(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
...
static DEVICE_ATTR(thub_status, 0664, show_thub_status, store_thub_status);
...
#endif
#endif
	BMT_status.UI_SOC = cm_get_StateOfCharge();
...
void mt_battery_GetBatteryData(void)
{
	...
	bat_vol = cm_get_Voltage(); 
	Vsense = battery_meter_get_VSense();
	if (upmu_is_chr_det() == KAL_TRUE)
		//ICharging = battery_meter_get_charging_current();
		ICharging = cm_get_AverageCurrent();
	else
		ICharging = 0;
	...
	temperature = cm_get_Temperature()+1;
	...
	BMT_status.SOC = cm_get_StateOfCharge();
	BMT_status.ZCV = ZCV;
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if ((upmu_is_chr_det() == KAL_TRUE) && (BMT_status.temperature >= batt_cust_data.max_charge_temperature)) {
		g_BatteryNotifyCode |= 0x0002;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
			    BMT_status.temperature);
	}
	else if ((upmu_is_chr_det() == KAL_FALSE) && (BMT_status.temperature >= 59)) {
		g_BatteryNotifyCode |= 0x0040;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) is high, show power off\n",
				BMT_status.temperature);
	}
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	else if ((upmu_is_chr_det() == KAL_TRUE) && (BMT_status.temperature < MIN_CHARGE_TEMPERATURE)) {
		g_BatteryNotifyCode |= 0x0020;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
			    BMT_status.temperature);
	}
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADC_CHANNEL_READ:
		...
		case BAT_THREAD_CTRL:
			...
			else if(battery_in_data[0]==2)
			{
				...
				printk("---@lgx----runin-----0097---%d,%d \n",battery_in_data[0],charging_enable);
				battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
			}
			else
			{
	...
	case Get_META_BAT_VOL:
		...
		adc_out_data[1] = g_emmc_raw_cid[0];
		adc_out_data[2] = g_emmc_raw_cid[1];
		adc_out_data[3] = hardware_info.tpd_fw_version; //temp_buf[8] 
		if (strncmp(hardware_info.cpu_name, "MT8161", 6) == 0){
			adc_out_data[4] = 2;
		} else if (strncmp(hardware_info.cpu_name, "MT8735", 6) == 0){
			adc_out_data[4] = 3;
		}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
        ...
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_Ycable_charging_enable);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_thub_status);
