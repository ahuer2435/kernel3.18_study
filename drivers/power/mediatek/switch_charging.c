	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			if (batt_cust_data.ac_charger_input_current != 0) {
				if (gpio_get_value(76) && (gpio_get_value(81))) {
					battery_log(BAT_LOG_CRTI, "[BATTERY] Sub camera is open and drop charger current !!!\n");
...
void pchr_turn_on_charging(void)
{
	...
	if (0x00 == BMT_status.bat_in_charging_enable)
	{
		charging_enable = KAL_FALSE;
	}
	...
	{
		bq24296_set_en_hiz(0x0);	// enable power path
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] g_runin_test_enter is Active, turn on charging !\r\n");
	}
