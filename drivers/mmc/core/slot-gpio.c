...
static irqreturn_t mmc_gpio_cd_irqt(int irq, void *dev_id)
{
	...
	// modified by Russell Guo, Mar-24, 2016
	// monitor external signal from IflyTek
	if (host->index == 1) {
		struct mmc_gpio *ctx = host->slot.handler_priv;
		if (ctx && ctx->cd_gpio) {
			void RobotCheersTH03VoiceNotify(const struct gpio_desc*);
			RobotCheersTH03VoiceNotify(ctx->cd_gpio);
		}
		return IRQ_HANDLED;
	}
