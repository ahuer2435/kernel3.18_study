...
#if CONFIG_GTP_SLIDE_WAKEUP  // liguangxian  20140917
static ssize_t gt91xxge_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char *p = page;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (DOZE_ENABLED == doze_status) {
			...
			printk("---@lgxtp----002----==%x,==%x,==%x,==%x\n",doze_buf[2],tpd->gesture_enablec,tpd->gesture_enables,tpd->gesture_enablez);
			if (ret > 0) {
				if (doze_buf[2] == 'c')
				{
