...
extern irqreturn_t kpd_slide_eint_handler(int irq, void *data);
#endif
int kpd_slide_irq;

int kpd_slide_qwerty_init(struct input_dev *dev)
{
#if KPD_HAS_SLIDE_QWERTY
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek, kpd-slide");
	kpd_slide_irq = irq_of_parse_and_map(node, 0);
	gpio_set_debounce(KPD_SLIDE_EINT, KPD_SLIDE_DEBOUNCE);
	ret =request_irq(kpd_slide_irq, (irq_handler_t) kpd_slide_eint_handler, IRQF_TRIGGER_LOW,
				"kpd-slide", NULL);

	//mt_eint_registration(KPD_SLIDE_EINT, true, 8, kpd_slide_eint_handler, false);
#if 0
