...
static unsigned int PINMUX_GPIO63__FUNC_GPIO63 = 63;  //R
static unsigned int PINMUX_GPIO61__FUNC_GPIO61 = 61;
...
static void audio_set_gpio_output(unsigned int GPIO, unsigned int output)
{
...
static ssize_t show_hardware_info(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(hardware_info.hq_lcm_name)       
        ...
        if(hardware_info.hq_tpd_name != NULL)
        {
                ...
                count += sprintf(buf + count, "ctp fw ver :0x%x\n", hardware_info.tpd_fw_version);
        }
        else
        {
...
*/
	...
	count += sprintf(buf+count, "CPU name:         ");//12 bytes
	if(hardware_info.cpu_name)
...
static ssize_t show_ctp(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if(hardware_info.hq_tpd_name != NULL)
	{
		count += sprintf(buf + count, "ctp driver  :%s , ctp sensor id  :%d , ctp fw ver  :0x%x\n", hardware_info.hq_tpd_name, hardware_info.tpd_sensor_id, hardware_info.tpd_fw_version);
	}
	else
	{
		count += sprintf(buf + count, "ctp driver NULL !\n");
...
//	printk("speaker control size %d\n", size);
	...
	if (size == 3) {
		...
		if (tmp[0] == '1') {
			...
			//mt_set_gpio_mode(PINMUX_GPIO63__FUNC_GPIO63, GPIO_MODE_00);
			audio_set_gpio_output(PINMUX_GPIO61__FUNC_GPIO61,1); 
			//audio_set_gpio_output(PINMUX_GPIO63__FUNC_GPIO63,0); 
			//mt_set_gpio_dir(GPIO_EXT_SPKAMP_EN_PIN, GPIO_DIR_OUT);
			//mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ONE); 
		} else {
		
		audio_set_gpio_output(PINMUX_GPIO61__FUNC_GPIO61,0);
		//audio_set_gpio_output(PINMUX_GPIO63__FUNC_GPIO63,0); 
			//mt_set_gpio_mode(GPIO_EXT_SPKAMP_EN_PIN, GPIO_MODE_00);
		//	mt_set_gpio_dir(GPIO_EXT_SPKAMP_EN_PIN, GPIO_DIR_IN);
			//mt_set_gpio_out(GPIO_SPEAKER_EN_PIN,GPIO_OUT_ZERO); 
		}

		if (tmp[1] == '1') {
			printk("enable right speaker\n");
						audio_set_gpio_output(PINMUX_GPIO63__FUNC_GPIO63,1); 
					//	audio_set_gpio_output(PINMUX_GPIO61__FUNC_GPIO61,0);
...
static ssize_t show_cpu(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    if(hardware_info.cpu_name)
        ret_value = sprintf(buf , "cpu name  :%s\n", hardware_info.cpu_name);
    else
        ret_value = sprintf(buf , "cpu name  :Not support\n");
...
static ssize_t show_timezone(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;
...
static ssize_t store_timezone(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);      
    return size;
}
...
static ssize_t show_cust_sn(struct device *dev,struct device_attribute *attr, char *buf)
{
    int ret_value = 1;

    ret_value = snprintf(buf , 13, (strstr(saved_command_line ,"sn=") + 3));
        
    return ret_value;
}
static ssize_t store_cust_sn(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    printk("[%s]: Not Support Write Function\n",__func__);      
    return size;
}
...
static DEVICE_ATTR(09_speaker, 0644, show_speaker, store_speaker);

static DEVICE_ATTR(13_wifi, 0644, show_wifi, store_wifi);
...
static DEVICE_ATTR(14_cpu, 0644, show_cpu, store_cpu);
static DEVICE_ATTR(15_timezone, 0644, show_timezone, store_timezone);
static DEVICE_ATTR(16_custsn, 0644, show_cust_sn, store_cust_sn);
...
static ssize_t hq_apk_debug_read1(struct file *file, char __user *buffer, size_t size, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
...
{       
	...
	if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_13_wifi)) != 0) goto exit_error;
	...
	if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_14_cpu)) != 0) goto exit_error;
	if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_15_timezone)) != 0) goto exit_error;
	if((ret_device_file = device_create_file(&(dev->dev), &dev_attr_16_custsn)) != 0) goto exit_error;
...
static int HardwareInfo_driver_remove(struct platform_device *dev)
{
	...
	device_remove_file(&(dev->dev), &dev_attr_13_wifi);
	...
	device_remove_file(&(dev->dev), &dev_attr_14_cpu);
	device_remove_file(&(dev->dev), &dev_attr_15_timezone);
	device_remove_file(&(dev->dev), &dev_attr_16_custsn);
