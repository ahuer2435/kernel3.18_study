...
#define VOL_1000 1000000
	...
	/* power ON */
	if (On) {
                	PK_DBG("[PowerON][ryf2]this is camera currSensorName: %s poweron!\n",currSensorName);
#if 1
		ISP_MCLK1_EN(0);
		ISP_MCLK2_EN(0);
		ISP_MCLK3_EN(0);
...
#endif
			mdelay(5);
		PK_DBG("[PowerON][ryf2]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
		...
		if (pinSetIdx == 0)
		...
		else if (pinSetIdx == 1 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_HI545_MIPI_RAW))){
			...
			*/
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
				PK_DBG("\n[CAMERA SENSOR] Fail to poweron camera!\n\n\n");
			...
			PK_DBG("[PowerOFF]pinSetIdx:%d\n", pinSetIdx);
                	PK_DBG("[PowerOFF][ryf2]this is camera currSensorName: %s PowerOFF!\n",currSensorName);
...
#define IDX_PS_OFF  3
	...
	if (On) {
		...
		if (pinSetIdx == 0)
			...
			ISP_MCLK2_EN(1);
#endif
                	PK_DBG("[ryf3]this is camera imx219 poweroff!\n");
