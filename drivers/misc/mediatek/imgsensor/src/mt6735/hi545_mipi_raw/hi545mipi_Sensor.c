...
void HI545_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 b_gain);

kal_uint16 HI545_Sensor_OTP_read(kal_uint16 otp_addr,kal_uint16 otp_addr_one,kal_uint16 otp_addr_two);

void HI545_Sensor_calc_wbdata(void);
...
static void HI545_OTP_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
kal_uint16 HI545_Sensor_OTP_read(kal_uint16 otp_addr,kal_uint16 otp_addr_one,kal_uint16 otp_addr_two)
{
    ...
    //mDELAY(10); //wangliangfeng 20141127 modify for solving sub-main camera switch time too long
...
void Hi545_Sensor_OTP_info(void)
{
	...
	{	
		...
		CalibrationVersion=HI545_Sensor_OTP_read(0x1803,0,0);
		...
		IRBGID = HI545_Sensor_OTP_read(0x180B,0,0);
		ColorTemperature=HI545_Sensor_OTP_read(0x180c,0,0);
		af_flag=HI545_Sensor_OTP_read(0x180d,0,0);
		lightsource_flag=HI545_Sensor_OTP_read(0x180e,0,0);
		CamerapositionID=HI545_Sensor_OTP_read(0x180f,0,0);
		reserved=HI545_Sensor_OTP_read(0x1810,0,0);
		infocheck = HI545_Sensor_OTP_read(0x1811,0,0);
    	break;
		case 0xd0:
		ModuleHouseID = HI545_Sensor_OTP_read(0x1812,0,0);
		CalibrationVersion=HI545_Sensor_OTP_read(0x1813,0,0);
		...
		IRBGID = HI545_Sensor_OTP_read(0x181B,0,0);
		ColorTemperature=HI545_Sensor_OTP_read(0x181c,0,0);
		af_flag=HI545_Sensor_OTP_read(0x181d,0,0);
		lightsource_flag=HI545_Sensor_OTP_read(0x181e,0,0);
		CamerapositionID=HI545_Sensor_OTP_read(0x180f,0,0);
		reserved=HI545_Sensor_OTP_read(0x1810,0,0);
		...
		CalibrationVersion=HI545_Sensor_OTP_read(0x1823,0,0);
		...
		IRBGID = HI545_Sensor_OTP_read(0x182B,0,0);
		ColorTemperature=HI545_Sensor_OTP_read(0x182c,0,0);
		af_flag=HI545_Sensor_OTP_read(0x182d,0,0);
		lightsource_flag=HI545_Sensor_OTP_read(0x182e,0,0);
		CamerapositionID=HI545_Sensor_OTP_read(0x180f,0,0);
		reserved=HI545_Sensor_OTP_read(0x1810,0,0);
	...
	checksum = (ModuleHouseID + CalibrationVersion+Year + Month + Day + SensorID + LensID + VCMID + DriverID+IRBGID+ColorTemperature+af_flag + lightsource_flag+reserved) % 0xFF + 1;
    ...
    LOG_INF("af_flag = %d \n", af_flag);
    LOG_INF("lightsource_flag = %d \n", lightsource_flag);
	
	printk("wangk_ModuleHouseID = %d \n", ModuleHouseID);
	printk("wangk_CalibrationVersion = %d \n", CalibrationVersion);
	printk("wangk_Year = %d, Month = %d, Day = %d\n", Year, Month, Day);
	printk("wangk_SensorID = %d \n", SensorID);
	printk("wangk_LensID = %d \n", LensID);
	printk("wangk_VCMID = %d \n", VCMID);
	printk("wangk_DriverID = %d \n", DriverID);
	printk("wangk_IRBGID = %d \n", IRBGID);
	printk("wangk_ColorTemperature = %d \n", ColorTemperature);
	printk("wangk_af_flag = %d \n", af_flag);
	printk("wangk_lightsource_flag = %d \n", lightsource_flag);
	printk("wangk_reserved = %d \n", reserved);
...
void HI545_Sensor_calc_wbdata(void)
{
   ...
   // printk("wb_unit_rg_h = 0x%d, wb_unit_rg_l = 0x%d, wb_unit_bg_h = 0x%d, wb_unit_bg_l = 0x%d, wb_golden_rg_h = 0x%d, wb_golden_rg_l = 0x%d, wb_golden_bg_h = 0x%d, wb_golden_bg_l = 0x%d, wb_unit_rg_l = 0x%d, wbcheck = 0x%d\n", 
	...
	if (checksum == wbcheck)
		...
		printk("wangk_HI545_Sensor: WB checksum PASS\n ");
		}
	else
		{
		LOG_INF("HI545_Sensor: WB checksum Fail\n ");
		printk("wangk_HI545_Sensor: WB checksum Fail\n ");
		}
	...
	rg_value = (((wb_unit_rg_h << 2) | ((wb_unit_rg_l>>6) & 0x0003)) & 0x03ff);
	bg_value = (((wb_unit_bg_h << 2) | ((wb_unit_bg_l>>6) & 0x0003)) & 0x03ff);
    ...
    printk("%s,After calcuate: g_gain = 0x%x, r_gain = 0x%x,b_gain = 0x%x\n",__func__, g_gain,r_gain,b_gain);
    HI545_Sensor_update_wb_gain( r_gain, b_gain);

}

void HI545_Sensor_update_wb_gain(kal_uint32 rg, kal_uint32 bg)
{
   // kal_uint16 r_gain=0, b_gain=0;
    ...
    HI545_OTP_write_cmos_sensor(0x0118, 0x01); //sleep Off
    HI545_OTP_write_cmos_sensor(0x050c, rg >> 8); //r_gain
    HI545_OTP_write_cmos_sensor(0x050d, rg & 0xFFFF); //r_gain
    HI545_OTP_write_cmos_sensor(0x050e, bg >> 8); //b_gain
    HI545_OTP_write_cmos_sensor(0x050f, bg & 0xFFFF); //b_gain
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0000, 0x0200); //orientation. [0]:x-flip, [1]:y-flip.
...
//	write_cmos_sensor(0x0008, 0x0B40); //line_length_h 2880
	...
	write_cmos_sensor(0x0A04, 0x013B); //TEST PATTERN
...
static void capture_setting(kal_uint16 currefps)
{
	...
	//---< Pixel Array Address >------//
	write_cmos_sensor(0x0012, 0x00AA); //x_addr_start_hact_h. 170 write_cmos_sensor(0x0018, 0x0ACB); //x_addr_end_hact_h. 2763 . 2763-170+1=2594 write_cmos_sensor(0x0026, 0x0018); //y_addr_start_vact_h. 24 write_cmos_sensor(0x002C, 0x07AF); //y_addr_end_vact_h.   1967. 1967-24+1=1944
	//---< Crop size : 2592x1944 >----//
	write_cmos_sensor(0x0128, 0x0002); //digital_crop_x_offset_l write_cmos_sensor(0x012A, 0x0000); //digital_crop_y_offset_l write_cmos_sensor(0x012C, 0x0A20); //2592 digital_crop_image_width write_cmos_sensor(0x012E, 0x0798); //1944 digital_crop_image_height 
	//---< FMT Size : 2592x1944 >---------//
	...
	//---< ETC set >---------------//
	write_cmos_sensor(0x0000, 0x0200); //orientation. [0]:x-flip, [1]:y-flip. write_cmos_sensor(0x0700, 0x0590); //140718
	write_cmos_sensor(0x001E, 0x0101); //140718
	write_cmos_sensor(0x0032, 0x0101); //140718
	write_cmos_sensor(0x0A02, 0x0100); //140718 Fast sleep Enable write_cmos_sensor(0x0A04, 0x011B); //TEST PATTERN enable
	write_cmos_sensor(0x0118, 0x0100); //sleep Off 		
		mDELAY(30);
 LOG_INF("E! currefps1111\n");
    } else {
		//////////////////////////////////////////////////////////////////////////
		//			Sensor        	 : Hi-545
		//      Set Name         : Capture
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[ryf6]i2c write id: 0x%x, sensor id: 0x%x\n\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("[ryf6]Read sensor id fail, write id: 0x%x, id: 0x%x\n\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[ryf8]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("[ryf8]Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
