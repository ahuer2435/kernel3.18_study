	...
	if (Group==0) //Group 1: Page2~Page3  0x3204~0x3243  , page 4:0x3204~0x3232
	{
		...
		if(retry<0)
			...
			for(address=0x3204 ; address<=0x3243 ; address++)
			{
					...
					//sum_value %= 255;
			CAM_CALDB("imx219 LSC otp In Group 1 page2 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
			}
			write_otp_reg(0x3300, 0x00);
			...
			for(address=0x3204 ; address<=0x3243 ; address++)
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
			CAM_CALDB("imx219 LSC otp In Group 1 page3 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
			write_otp_reg(0x3300, 0x00);
			...
			for(address=0x3204 ; address<=0x3232 ; address++)//3232 is last lsc address of group1
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
			CAM_CALDB("imx219 LSC otp In Group 1 page4 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
			...
			for(address=0x3222 ; address<=0x3243 ; address++)
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
					CAM_CALDB("imx219 LSC otp In Group 2 page7 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
			write_otp_reg(0x3300, 0x00);
			...
			for(address=0x3204 ; address<=0x3243 ; address++)
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
					CAM_CALDB("imx219 LSC otp In Group 2 page8 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
			write_otp_reg(0x3300, 0x00);
			...
			for(address=0x3204 ; address<=0x3243 ; address++)
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
					CAM_CALDB("imx219 LSC otp In Group 2 page9 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
			write_otp_reg(0x3300, 0x00);
			...
			for(address=0x3204 ; address<=0x3210 ; address++)
				{
				    sum_value += read_otp_reg(address);
					//sum_value %= 255;
					CAM_CALDB("imx219 LSC otp In Group 2 page10 address=0x%0x,lscdata =%0x\n",address, read_otp_reg(address));
				}
...
//Set input clock frequency
	...
	for(address=0x3204 ; address<=0x3243 ; address++)//3232 is last lsc address of group1
		{
			CAM_CALDB("imx219 otp page0 address=0x%0x,data =%0x\n",address, read_otp_reg(address));
		}
		    ...
		    if (Vaild_Group==0)
		    	{
					...
					Group1_Checksum_Value = read_otp_reg(0x3241);//checksum1 of lsc
					
					CAM_CALDB("imx219 LSC otp Group 1 0x3241 Group1_Checksum_Value=%d \n",Group1_Checksum_Value);
					if(Checksum_Value == Group1_Checksum_Value)
						{
			...
			else if (Vaild_Group==2)
		    	{
					...
					if(Checksum_Value == Group2_Checksum_Value)
						{
...
CAM_CALDB("[S24CAM_CAL] LONG TEST SIZE=%ld.\n",sizeof( stCAM_CAL_INFO_STRUCT));
    }
