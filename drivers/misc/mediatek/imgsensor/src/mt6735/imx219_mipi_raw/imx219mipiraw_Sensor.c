...
#define IMX219OTP_ENABLE
#ifdef IMX219OTP_ENABLE
	extern int IMX219_UpdateLSC(MUINT8 update_sensor_otp_awb, MUINT8 update_sensor_otp_lsc,MUINT8 module_types);//sunny module_types:1 qtech module_types:0
	extern int IMX219_LSC_Vaild_Group;
#endif 
...
if(imgsensor.update_sensor_otp_lsc != 0)
		{
		if(IMX219_LSC_Vaild_Group==-1)
			{
				LOG_INF(" this Module has not lsc data IMX219_LSC_Vaild_Group: 0x%x\n", IMX219_LSC_Vaild_Group);	  
			}		
		else
			{
				//write_cmos_sensor(0x0190, 0x01);
				//write_cmos_sensor(0x0191,0x00);
				//write_cmos_sensor(0x0192, IMX219_LSC_Vaild_Group);
				//write_cmos_sensor(0x0193, 0x00);
				//write_cmos_sensor(0x01A4, 0x03);
			}
		}
	else {
		write_cmos_sensor(0x0190,0x00);//	
		write_cmos_sensor(0x0191,0x00);//
	}
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	    ...
	    IMX219_UpdateLSC(imgsensor.update_sensor_otp_awb, imgsensor.update_sensor_otp_lsc,0);
	#endif 
	return ERROR_NONE;
}	/*	open  */
...
case SENSOR_FEATURE_SET_SENSOR_OTP_AWB_CMD:
		...
		case SENSOR_FEATURE_SET_SENSOR_OTP_LSC_CMD:
			...
			spin_lock(&imgsensor_drv_lock);
			imgsensor.update_sensor_otp_lsc = (BOOL)*feature_data_16;
			spin_unlock(&imgsensor_drv_lock);
