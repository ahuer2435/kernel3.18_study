	...
	if (g_pSensorFunc) {
		...
		if (ERROR_NONE != err) {
            ...
            if (DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[0]){
                memcpy(hardware_info.mainCameraName,(char*)g_invokeSensorNameStr[0],CAM_NAME_LEN);
            }
			else if (DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[1]) {
				memcpy(hardware_info.subCameraName,(char*)g_invokeSensorNameStr[1],CAM_NAME_LEN);
            }
		}
	} else {
