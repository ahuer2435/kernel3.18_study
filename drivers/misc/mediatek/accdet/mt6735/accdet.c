...
static unsigned int GPIO_HEADSET_SWITCH = 5;
...
/*Disable ACCDET*/
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
...
#endif
		#ifdef CONFIG_HQ_A6604_WIFIONLY_PROJECT
		msleep(250);//80  200 /// @lgx 
		 #endif
		accdet_init();	/* do set pwm_idle on in accdet_init*/
		...
		msleep(80);//80
		#ifdef CONFIG_HQ_A6604_WIFIONLY_PROJECT
		msleep(400);//liguangxian_1108
		 #endif
    	accdet_adc = Accdet_PMIC_IMM_GetOneChannelValue(1);

        printk("[Accdet]adc accdet_voltage = %d mv\n", accdet_adc);

        if(accdet_adc < 500)//<500
        {	
            msleep(10);
    	    gpio_set_value(GPIO_HEADSET_SWITCH,1);
    	    msleep(20);
        }
        else
        {
            msleep(10);
    	    gpio_set_value(GPIO_HEADSET_SWITCH,0);
    	    msleep(20);
        }
	} else {
/*EINT_PIN_PLUG_OUT*/
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	gpio_direction_output(GPIO_HEADSET_SWITCH, 0);
	gpio_set_value(GPIO_HEADSET_SWITCH, 0);
