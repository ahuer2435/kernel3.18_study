...
const unsigned int VBAT_CV_VTH[] = {
	...
	4336000, 4352000, 4368000, 4378000
};
...
static unsigned int charging_hw_init(void *data)
{
	...
	bq24296_set_iterm(0x1);	/* Termination current 128mA */

	if (batt_cust_data.high_battery_voltage_support)
		bq24296_set_vreg(0x36);	/* VREG 4.352V */
	else
...
static unsigned int charging_get_charger_type(void *data)
{
	...
	*(CHARGER_TYPE *) (data) = hw_charging_get_charger_type();
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"ACA_type =%d\r\n",i_ACA_type);
    if(i_ACA_type == 0x4)
